...
!   print*, 'Hello from rank ', rank, 'of ', nprocs
    ...
    ! tempvar(2,1) = 80
    ! tempvar(2,2) = 100
    ! CALL densityprofilefortraninterop(tempvar)
...
SUBROUTINE TRACE_RKCK(params, rayhour, rayazim, rayelev, edensARR, edensTHT, dip, hfrays, hfranges, hfionos, &
  ...
  real*4::        edens, edensUP, vedens, nr2, dnr2dr, edensMax, nr2refl
  ...
  real*4,dimension(10)::       ranout
  real*4,dimension(10,5000)::  ionosout
  ...
  nr2 = 1.
  dnr2dr = 0.
  ! Initialize refractive index at refl altitude for gscat
  nr2refl = 1.
...
!  ransave(1) = grpran
  ...
  do while (ihop.lt.params%nhop.and.r.lt.(Rav + 500.)*1e3.and.theta.lt.edensTHT(500).and.r.ge.Rav*1e3.and.nrstep.lt.5000)
    ...
    if (rtmp-r.le.0..and.rtmp-rrefl.ge.0.) then
        rrefl = rtmp
        nr2refl = nr2
    endif
    ...
    if (rtmp*1e-3.le.Rav) then
      ...
      ranout = (/rayhour, rayazim, rayelev, real(ihop+1), rrefl, thetatmp, grpran+h, latiout, longiout, sqrt(nr2refl)/) ! , (ransave(nrstep-1) + sqrt(nr2)*h)
      CALL MPI_FILE_WRITE_SHARED(hfranges, ranout, 10, MPI_REAL, status, code)
...
!        asp_ran = ransave(nrstep-1) + h/2.*sqrt(nr2)
        ...
        ionosout(1:10,naspstep) = (/asp_alt, asp_theta, asp_grpran, ranelev, asp_w, sqrt(nr2), latiin, longiin, h, real(ihop+0.5)/)
        naspstep = naspstep + 1
      endif
...
!                                      (ransave(n),n=1,nrstep-1), &
  ...
  CALL MPI_FILE_WRITE_SHARED(hfionos, (/real(naspstep-1), &
                                      ...
                                      (ionosout(9,n),n=1,naspstep-1), &
                                      (ionosout(10,n),n=1,naspstep-1)/), 1 + 3 + 10*(naspstep-1), MPI_REAL, status, code)
...
SUBROUTINE IRI_ARR(params, hour, azim, edensARR, edensPOS, edensTHT, dip)
    ...
    real*4,dimension(500):: echaimOut
    ...
    real*4:: year_parse,month_parse,day_parse,hour_parse,min_parse,sec_parse

    year_parse = params%year
    month_parse =floor(params%mmdd / 100.)
    day_parse = params%mmdd - month_parse*100
    hour_parse = floor(hour - 25)
    min_parse = floor((hour - 25 - hour_parse)*60)
    sec_parse = 0
...
! Calling IRI subroutine
  ...
  ! print *, params%year , params%mmdd, hour
  ! print *, year_parse,month_parse,day_parse, &
  ! hour_parse,min_parse,sec_parse

! Calling ECHAIM subroutine
    call densityprofilefortraninterop(edensPOS(1,1),edensPOS(1,2),year_parse,month_parse,day_parse, &
    hour_parse,min_parse,sec_parse,echaimOut)

    do j=1,500
        edensARR(j,1) = echaimOut(j)!outf(1,j)

        !print the difference
       !print *, outf(1,j) ,echaimOut(j)
        !Print "(i5,e13.6)", j ,echaimOut(j) - outf(1,j)
    enddo
...
!                edensPOS(n,1), edensPOS(n,2))
        ...
        !echaim to get electron density
        call densityprofilefortraninterop(edensPOS(n,1),edensPOS(n,2),year_parse,month_parse,day_parse, &
        hour_parse,min_parse,sec_parse,echaimOut)

        do j=1,500
            edensARR(j,n) = echaimOut(j)!outf(1,j)
        enddo
